  ...
  return (
    ...
    >
      <div className="relative">
        <div className="overflow-hidden rounded-[1.6rem] bg-white shadow-[0_30px_70px_-24px_rgba(189,24,97,0.35)] ring-1 ring-[#f6c9dc]">
          <div ref={track} className="flex transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]">
            {SLIDES.map((s, i) => (
              <div key={s.src} className="relative aspect-[3/4] w-full shrink-0 sm:aspect-auto sm:h-[min(70vh,680px)]">
                ...
                />
                <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#2d0a1f]/85 via-[#2d0a1f]/35 to-transparent p-5 pb-6 pt-16 sm:p-7 sm:pb-8 sm:pt-24">
                  <span className="rounded-full bg-white/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[#ffb3d1] backdrop-blur">
                    {s.tag}
                  </span>
        ...
        <button
          ...
          className="group absolute left-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[#2d0a1f] shadow-md ring-1 ring-[#f6c9dc] backdrop-blur transition hover:bg-white hover:text-[#c00e62] sm:left-5"
        >
        ...
        <button
          ...
          className="group absolute right-3 top-1/2 z-10 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[#2d0a1f] shadow-md ring-1 ring-[#f6c9dc] backdrop-blur transition hover:bg-white hover:text-[#c00e62] sm:right-5"
        >
        ...
        {SLIDES.map((_, i) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              i === index ? "w-7 bg-[#ec1478]" : "w-2 bg-[#f6c9dc] hover:bg-[#f087b6]"
            }`}
          />
        ))}
      </div>
      <p className="mt-3 text-center text-xs font-medium text-[#967184]">
