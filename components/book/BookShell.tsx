...
function TOCButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-2 rounded-full border border-[#d1d5db] bg-white px-4 py-2 text-[13px] font-bold text-[#3f6f86] shadow-sm transition hover:border-[#3b82f6]"
...
export default function BookShell({
  ...
  const pageHeadCss = useMemo(() => {
    const box = (t: string) =>
      `@top-right { content: ${JSON.stringify(t)}; font-family: "Plus Jakarta Sans", system-ui, sans-serif; font-size: 6.5pt; font-weight: 600; letter-spacing: 0.03em; color: #9ca3af; white-space: nowrap; }`;
  ...
  const toc = (
    <nav className="toc-rail" aria-label="Contents">
      <div className="mb-5 hidden items-center gap-3 md:flex">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-[#3b82f6] to-[#2563eb] text-[15px] font-bold text-white">
          SR
        </span>
        <div className="min-w-0">
          <p className="truncate text-[13px] font-bold text-[#eaf0f6]">{meta.railTitle ?? meta.title}</p>
          <p className="text-[11px] text-[#93c5fd]">{meta.railEdition ?? meta.edition}</p>
        </div>
      </div>
      <p className="mb-1 hidden text-[10px] font-bold uppercase tracking-[0.18em] text-[#93c5fd] md:block">
        Contents
      </p>
          ...
          <li key={s.id}>
            <button
              ...
              className={`group flex w-full items-start gap-2 rounded-lg px-2.5 py-2 text-left transition ${
                active === s.id ? "bg-[#3b82f6]/15 text-[#eaf0f6]" : "text-[#9ca3af] hover:bg-white/5 hover:text-white"
              }`}
              aria-current={active === s.id ? "true" : undefined}
            >
              <span className="mt-px w-6 shrink-0 text-right text-[11px] font-bold text-[#93c5fd]">
                {s.num != null ? (s.num !== "" ? s.num : "·") : i + 1}
              </span>
              <span className="min-w-0">
                <span className="block truncate text-[13px] font-semibold">{s.title}</span>
                <span className="block truncate text-[11px] text-[#93c5fd]">{s.part}</span>
              </span>
            </button>
          </li>
        ))}
      ...
      <div className="mt-6 hidden rounded-xl border border-white/10 bg-white/5 p-3 md:block">
        <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#eaf0f6]">
          Reading time
        </p>
        <p className="mt-1 text-2xl font-bold text-white">{Math.round(totalMin)}<span className="text-sm font-semibold text-[#93c5fd]"> min</span></p>
  ...
  const printToc = (
    <section id="contents" className="book-toc hidden rounded-none bg-white px-10 py-16 print:block" style={{ page: "toc-page" }}>
      <p className="text-[10px] font-bold uppercase tracking-[0.26em] text-[#2563eb]">
        {meta.edition}
      </p>
      <h2 className="mt-3 font-display text-[40px] font-semibold tracking-tight text-[#2f5d73]">
        Contents
      </h2>
      <p className="mt-2 max-w-[52ch] text-[13px] leading-relaxed text-[#4b5563]">
        {meta.tocIntro ??
          ...
          return (
            <div key={p.index}>
              <p className="mb-2 flex items-baseline gap-3">
                <span className="text-[9.5px] font-bold uppercase tracking-[0.22em] text-[#2563eb]">
                  {partLabel(p.index)}
                </span>
                ...
                {group.map((s) => {
                  ...
                  return (
                    <li key={s.id}>
                      <a
                        href={`#${s.id}`}
                        className="toc-row grid grid-cols-[2em_1fr_1.4em] items-baseline gap-1 text-[13px] text-[#2f5d73] no-underline hover:text-[#3b82f6]"
                      >
                        <span className="text-right font-bold text-[#3b82f6]">{s.num != null ? (s.num !== "" ? s.num : "·") : global + 1}</span>
                        <span className="font-semibold">{s.title}</span>
                        <span
                          className="toc-dots text-right text-[13px] font-bold text-[#3f6f86]"
                          data-page-for={s.id}
                          data-page=""
                        />
                      </a>
  ...
  return (
    <div className="book-shell min-h-screen bg-[#f8fafc]">
      <style dangerouslySetInnerHTML={{ __html: pageHeadCss }} />

      {/* progress bar */}
      <div className="fixed inset-x-0 top-0 z-[60] h-1 bg-[#eaf0f6]">
        <div
          className="h-full bg-gradient-to-r from-[#3b82f6] to-[#2563eb] transition-[width] duration-150"
          style={{ width: `${progress}%` }}
        />
      </div>

      {/* mobile top bar */}
      <header className="book-header sticky top-0 z-50 flex items-center justify-between gap-3 border-b border-[#d1d5db] bg-[#f8fafc]/90 px-4 py-3 backdrop-blur md:hidden">
        <div className="flex items-center gap-2">
          <span className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-[#3b82f6] to-[#2563eb] text-[12px] font-bold text-white">
            SR
          </span>
          <p className="text-[13px] font-bold text-[#2f5d73]">{meta.title}</p>
        </div>
        <TOCButton onClick={() => setOpen(true)} />
      </header>

      {/* desktop rail */}
      <aside className="book-rail fixed left-0 top-0 z-40 hidden h-dvh w-[300px] flex-col overflow-y-auto bg-[#1f2933] px-4 py-6 md:flex">
        {toc}
      </aside>
      ...
      <div className={`fixed inset-0 z-[70] md:hidden ${open ? "" : "pointer-events-none"}`} aria-hidden={!open}>
        <div
          className={`absolute inset-0 bg-[#1f2933]/60 backdrop-blur-sm transition-opacity ${open ? "opacity-100" : "opacity-0"}`}
          onClick={() => setOpen(false)}
        />
        <div
          className={`absolute inset-y-0 left-0 w-[85%] max-w-[320px] overflow-y-auto bg-[#1f2933] px-4 py-6 transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}
          role="dialog"
          aria-label="Contents"
        >
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm font-bold text-[#eaf0f6]">Contents</p>
        ...
        <div className="mx-auto max-w-[760px]">
          ...
          <div className="mb-8 hidden items-center justify-between md:flex">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-[#d1d5db] bg-white px-4 py-2 text-[13px] font-bold text-[#3f6f86] transition hover:border-[#3b82f6]"
            >
              <svg viewBox="0 0 20 20" className="h-4 w-4 fill-current">
          ...
          {/* cover */}
          <section id="cover" className="book-cover rounded-3xl bg-gradient-to-br from-[#f5f7f9] via-[#eaf0f6] to-[#dbeafe] px-6 py-14 text-center shadow-[0_24px_70px_-30px_rgba(59, 130, 246, 0.45)] sm:px-10 sm:py-20">
            <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#2563eb]">
              {meta.edition}
            </p>
            <h1 className="mx-auto mt-4 max-w-[520px] text-balance font-display text-4xl font-semibold leading-[1.05] tracking-tight text-[#2f5d73] sm:text-6xl">
              {meta.title}
            </h1>
            <p className="mx-auto mt-4 max-w-[440px] text-[15px] leading-relaxed text-[#4b5563] sm:text-[17px]">
              {meta.subtitle}
            </p>
            <p className="mt-6 text-[13px] font-bold uppercase tracking-[0.18em] text-[#2563eb]">
              {meta.author}
            </p>
            {meta.priceLine && <div className="mt-6">{meta.priceLine}</div>}
            <div className="mx-auto mt-8 flex max-w-md flex-wrap items-center justify-center gap-2 text-[11.5px] font-semibold text-[#3f6f86]">
              {(meta.chips ?? DEFAULT_CHIPS).map((t) => (
                <span key={t} className="rounded-full border border-[#d1d5db] bg-white/70 px-3 py-1">
                  {t}
                </span>
              ))}
            </div>
          ...
          {sections.map((s, i) => {
            ...
            return (
              <article
                ...
                className={`book-chapter scroll-mt-24 border-t border-[#d1d5db] py-12 md:scroll-mt-10 md:py-16 ${
              ...
              >
                ...
                <div className="mb-6 flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-[#3b82f6] px-3 py-1 text-[10.5px] font-bold uppercase tracking-[0.14em] text-white">
                    {s.part}
                  </span>
                  <span className="rounded-full bg-[#f5f7f9] px-3 py-1 text-[10.5px] font-bold uppercase tracking-[0.14em] text-[#9ca3af]">
                    {s.minutes} min read
                  </span>
                </div>
<h2 className="text-balance font-display text-3xl font-semibold tracking-tight text-[#2f5d73] sm:text-[34px]">
              {s.num != null && s.num !== "" && (
                <span className="mr-2 text-[#3b82f6]">{s.num}.</span>
              )}
              {s.title}
            </h2>
                <p className="mt-2 max-w-[60ch] text-[15px] text-[#9ca3af]">{s.blurb}</p>
                <div className="book-body mt-6">
                  {s.body}
                  <BookEndCard chapterNum={i + 1} total={sections.length} partName={s.part} pages={meta.pages} />
                </div>

                <div className="book-nav mt-12 flex items-center justify-between gap-3 border-t border-[#d1d5db] pt-6">
                  <button
                    onClick={() => idx(i - 1)}
                    disabled={i === 0}
                    className="rounded-full border border-[#d1d5db] bg-white px-4 py-2 text-[13px] font-bold text-[#3f6f86] transition enabled:hover:border-[#3b82f6] disabled:opacity-40"
                  >
                    ← Previous
                  </button>
                  <p className="hidden text-[12px] font-semibold text-[#9ca3af] sm:block">
                    {i + 1} / {sections.length}
                  </p>
                  <button
                    onClick={() => idx(i + 1)}
                    disabled={i === sections.length - 1}
                    className="rounded-full bg-gradient-to-r from-[#3b82f6] to-[#2563eb] px-5 py-2 text-[13px] font-bold text-white shadow-[0_10px_24px_-10px_rgba(59, 130, 246, 0.7)] transition enabled:hover:brightness-105 disabled:opacity-40"
                  >
                    Next →
                  </button>
                </div>
          ...
          {/* back cover */}
          <section className="book-back rounded-3xl bg-[#2f5d73] px-6 py-14 text-center text-[#eaf0f6] sm:px-10">
            <p className="text-6xl leading-none text-[#3b82f6]">“</p>
            ...
            <p className="mt-6 text-[12px] font-bold uppercase tracking-[0.2em] text-[#93c5fd]">
